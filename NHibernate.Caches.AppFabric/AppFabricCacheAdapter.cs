...
namespace NHibernate.Caches.AppFabric
{
    ...
    public abstract class AppFabricCacheAdapter : ICache
    {
        ...
        protected internal string EffectiveLockRegionName
        {
            get { return AppFabricProviderSettings.Settings.RegionPrefix + LocksRegionName; }
        }

        protected internal string EffectiveAppFabricRegionName
        {
            get { return AppFabricProviderSettings.Settings.RegionPrefix + AppFabricRegionName; }
        }
        ...
        public virtual void Clear()
        {
            try
            {
                Cache.ClearRegion(EffectiveAppFabricRegionName);
            }
        ...
        public virtual void Destroy()
        {
            try
            {
                Cache.RemoveRegion(EffectiveAppFabricRegionName);
            }
            catch (DataCacheException ex)
            {
                if (ex.ErrorCode != DataCacheErrorCode.RegionDoesNotExist)
        ...
        public virtual object Get(object key)
        {
            ...
            try
            {
                var value = Cache.Get(key.ToString(), EffectiveAppFabricRegionName);
        ...
        private void Lock(object key, bool createMissingRegion)
        {
            ...
            try
            {
                _locksCache.GetAndLock(key.ToString(), TimeSpan.FromMilliseconds(Timeout), out lockHandle, EffectiveLockRegionName, true);
        ...
        private void Put(object key, object value, bool createMissingRegion)
        {
            ...
            try
            {
                ...
                Cache.Put(key.ToString(), value, EffectiveAppFabricRegionName);
            }
            catch (DataCacheException ex)
            {
                if (ex.ErrorCode == DataCacheErrorCode.RegionDoesNotExist && createMissingRegion)
        ...
        public virtual void Remove(object key)
        {
            ...
            try
            {
                ...
                {
                    Cache.Remove(key.ToString(), EffectiveAppFabricRegionName);
                }
            }
            catch (DataCacheException ex)
            {
        ...
        public virtual void Unlock(object key)
        {
            try
            {
                ...
                {
                    _locksCache.Unlock(key.ToString(), _lockHandles[key.ToString()], EffectiveLockRegionName);
        ...
        {
            CreateRegion(_locksCache, EffectiveLockRegionName, callback);
        }
        ...
        {
            CreateRegion(Cache, EffectiveAppFabricRegionName, callback);
        }
